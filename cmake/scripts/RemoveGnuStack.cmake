# =============================================================================
# RemoveGnuStack.cmake - Remove PT_GNU_STACK from ELF (run via cmake -P)
# =============================================================================
# Usage: cmake -DELF_FILE=<path> -P RemoveGnuStack.cmake
#
# Safety net for Solaris builds: if the linker emits a PT_GNU_STACK program
# header despite -z nognustack, this script patches the p_type field to
# PT_NULL (0) so the Solaris kernel ignores it. PT_GNU_STACK (0x6474e551)
# falls in the OS-specific range and Solaris rejects it with ENOEXEC.
#
...
# PT_GNU_STACK = 0x6474e551, little-endian = 51 e5 74 64
set(_gnu_stack_le "51e57464")
...
while(_i LESS _e_phnum)
    ...
    if(_ptype_hex STREQUAL "${_gnu_stack_le}")
        set(_found TRUE)
        # Patch p_type from PT_GNU_STACK to PT_NULL
        if(CMAKE_HOST_WIN32)
            execute_process(
        ...
        if(NOT _result EQUAL 0)
            message(FATAL_ERROR "Failed to patch PT_GNU_STACK in: ${ELF_FILE}")
        endif()

        message(STATUS "Patched PT_GNU_STACK -> PT_NULL at program header ${_i} in: ${ELF_FILE}")
        break()
    endif()
...
if(NOT _found)
    message(STATUS "No PT_GNU_STACK found in: ${ELF_FILE} (OK)")
endif()
