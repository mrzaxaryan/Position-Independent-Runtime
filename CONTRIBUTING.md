...
`Result::Err` has three overloads for chainable error types:

```cpp
// Single runtime code (guard failure, no OS call):
...
**Windows NTDLL failure** — ntdll.cc packages the `NTSTATUS`; callers propagate with `Err(failedResult, code)`:
```cpp
