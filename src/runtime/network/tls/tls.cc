...
Result<void, Error> TlsClient::SendPacket(INT32 packetType, INT32 ver, TlsBuffer &buf)
{
    ...
    *(UINT16 *)(tempBuffer.GetBuffer() + bodySizeIndex) = ByteOrder::Swap16(tempBuffer.GetSize() - bodySizeIndex - 2);
...
Result<void, Error> TlsClient::SendClientHello(const CHAR *host)
{
    ...
    *(PUINT16)(sendBuffer.GetBuffer() + cipherCountIndex) = ByteOrder::Swap16(sendBuffer.GetSize() - cipherCountIndex - 2);
    ...
    if (hastls13)
    {
        ...
        for (INT32 i = 0; i < ECC_COUNT; i++)
        {
            ...
            *(UINT16 *)(sendBuffer.GetBuffer() + shareSizeSub) = ByteOrder::Swap16(sendBuffer.GetSize() - shareSizeSub - 2);
        }
        *(UINT16 *)(sendBuffer.GetBuffer() + shareSize) = ByteOrder::Swap16(sendBuffer.GetSize() - shareSize - 2);
        *(UINT16 *)(sendBuffer.GetBuffer() + shareSize + 2) = ByteOrder::Swap16(sendBuffer.GetSize() - shareSize - 4);
    }
    LOG_DEBUG("Appending ClientHello with extensions, size: %d bytes", sendBuffer.GetSize() - extSizeIndex - 2);

    *(UINT16 *)(sendBuffer.GetBuffer() + extSizeIndex) = ByteOrder::Swap16(sendBuffer.GetSize() - extSizeIndex - 2);
    sendBuffer.GetBuffer()[handshakeSizeIndex] = 0;
    *(UINT16 *)(sendBuffer.GetBuffer() + handshakeSizeIndex + 1) = ByteOrder::Swap16(sendBuffer.GetSize() - handshakeSizeIndex - 3);
...
Result<void, Error> TlsClient::OnServerHello(TlsBuffer &reader)
{
    ...
    {
        SSL_EXTENTION type = (SSL_EXTENTION)ByteOrder::Swap16(reader.Read<INT16>());
