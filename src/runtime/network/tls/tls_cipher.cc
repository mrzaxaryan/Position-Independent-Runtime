...
{
    if (!this->isEncoding || keepOriginal)
    {
    ...
    *((UINT16 *)(aad + 3)) = ByteOrder::Swap16(ChaCha20Encoder::ComputeSize(packetSize, CipherDirection::Encode)); //-header_size
    UINT64 clientSeq = ByteOrder::Swap64(this->clientSeqNum++);
...
Result<void, Error> TlsCipher::Decode(TlsBuffer &inout, INT32 version)
{
    ...
    *((UINT16 *)(aad + 3)) = ByteOrder::Swap16(inout.GetSize()); //-header_size
    UINT64 serverSeq = ByteOrder::Swap64(this->serverSeqNum++);
    ...
    auto setSizeResult = inout.SetSize(this->decodeBuffer.GetSize());
    if (!setSizeResult)
